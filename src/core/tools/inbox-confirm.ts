...
export const inboxConfirmTools: ToolDef[] = [
  {
    ...
    run: async (_port, args, workspaceId, ctx) => {
      ...
      return {
        ...
        confirmToken,
        expiresAt,
        next: 'Show the user which comment this is (author and text) and exactly what will be sent or deleted, and wait for their yes in the conversation, even if they already gave you the text. Only then call confirm_inbox_action with this confirmToken and the same itemId, action and text, plus the same workspaceId only if you passed one.',
      };
    },
  },
  {
